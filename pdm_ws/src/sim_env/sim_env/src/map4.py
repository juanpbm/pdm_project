...
class Panda_Sym:
    def __init__(self, render=True,
        ...
        self.init_pos = np.array([4.0,-1.0,0.0])
        self.env: UrdfEnv = None
        self.Gen_Env()
        img = self.capture_overhead_image(self.env)

        cv.imwrite("home/adi/Desktop/maps/Map.png", img)
...
# OCCUPANCY MAP SECTION ENDS
        ...
        room1_obs_dicts = [
            {
                ...
                'geometry': {
                    'position': [self.boundary_size - self.room_size, 0.0 - (self.room_size-self.door_size)/2 - 11, self.height / 2.0],
            ...
            },
            {
                ...
                "rgba": [0.0, 0.0, 1.0, 1.0],
                'geometry': {
                    'position': [self.boundary_size - self.room_size/2 +2/2, 0.0 - self.room_size+self.door_size-6, self.height / 2.0],
        ...
        room2_obs_dicts = [
            {
                ...
                'geometry': {
                    'position': [(self.room_size), -(16 - (self.room_size-self.door_size)/2)+11, self.height / 2.0],
                    'width': self.room_size-self.door_size, 'height': self.height, 'length': 0.1
                }
            },
            {
                ...
                "rgba": [1.0, 0.0, 0.0, 1.0],
                'geometry': {
                    'position': [self.room_size/2-2/2, -(16 - self.room_size + self.door_size)+6, self.height / 2.0],
                    'width': 0.1, 'height': self.height, 'length': self.room_size-4/2
                }
            },
        ]
    ...
    def Gen_Shelf(self):
        self.shelf1 = BoxObstacle(name='obstacle1', content_dict=(
                ...
                'geometry': {
                    'position': [self.boundary_size/2 -9.0, -9.5, self.height / 2.0], 
        ...
        self.shelf2 = BoxObstacle(name='obstacle1', content_dict=(
                ...
                "rgba": [0.5, 0.5, 0.0, 1.0],
                'geometry': {
                    'position': [self.boundary_size/2+4.0, -3.0, self.height / 2.0], 
                    'width':  4, 'height': self.height, 'length': self.obstacle_size+0.5
                }
            }))
        ...
        self.shelf3 = BoxObstacle(name='obstacle1', content_dict=(
                ...
                "rgba": [0.5, 0.5, 0.0, 1.0],
                'geometry': {
                    'position': [self.boundary_size/2 -5.0, -9.5, self.height / 2.0], 
                    'width':  4, 'height': self.height, 'length': self.obstacle_size+0.5
                }
            }))
        ...
        self.shelf4 = BoxObstacle(name='obstacle1', content_dict=(
                ...
                "rgba": [0.5, 0.5, 0.0, 1.0],
                'geometry': {
                    'position': [self.boundary_size/2 -9.0, -14.0, self.height / 2.0], 
                    'width':  4, 'height': self.height, 'length': self.obstacle_size+0.5
                }
            }))
        ...
        self.shelf5 = BoxObstacle(name='obstacle1', content_dict=(
                ...
                "rgba": [0.5, 0.5, 0.0, 1.0],
                'geometry': {
                    'position': [self.boundary_size/2+6.0, -5.0, self.height / 2.0], 
                    'width':  4, 'height': self.height, 'length': self.obstacle_size+0.5
                }
            }))
        ...
        self.shelf6 = BoxObstacle(name='obstacle1', content_dict=(
                ...
                "rgba": [0.5, 0.5, 0.0, 1.0],
                'geometry': {
                    'position': [self.boundary_size/2 -5.0, -14.0, self.height / 2.0], 
                    'width':  4, 'height': self.height, 'length': self.obstacle_size+0.5
                }
            }))
