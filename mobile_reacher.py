...
def end_effector_matrix_original(robot_joints):
    end_effector_matrix=np.eye(4)
    for x in range(len(robot_joints)-2):
        joint_type = robot_joints[x].joint_type
        joint_limits = robot_joints[x].limit
        print(f"{robot_joints[x].name:<20} {joint_type:<15} {x:<10}")
        print("Axis:")
        print(robot_joints[x].axis)
        # print(f"Lower Limit: {joint_limits.lower:<15} Upper Limit: {joint_limits.upper:<10}")
        # print(np.round(robot_joints[x].origin,5))
        end_effector_matrix=end_effector_matrix@np.round(robot_joints[x].origin,5)
    print("End effector matrix")   
...
def run_mobile_reacher(n_steps=10000, render=False, goal=True, obstacles=True):
    ...
    end_effector_matrix_original(joints)
    ...
    print(ja)
    history = []
    target_xyz = np.array([0.5, 0.4, 0.5])
    max_velocity = 0.5
